...
describe('execution fabric SQLite schema and worker lifecycle', () => {
  it('opens schema v4 with parsed-output and artifact indexes, foreign keys, WAL, integrity, and idempotent reopen', async () => {
    const f = await fixture('open');
    const store = new ExecutionStore();
    stores.push(store);
    const opened = await store.open({ dbPath: f.dbPath, busyTimeoutMs: 1_500 }) as any;
    expect(opened).toMatchObject({ priorUserVersion: 0, schemaVersion: 4, quickCheck: 'ok', integrity: 'ok' });
    expect(EXECUTION_SCHEMA_VERSION).toBe(4);
    const status = await store.status();
    expect(status).toMatchObject({ healthy: true, schemaVersion: 4, integrity: 'ok', dbPath: f.dbPath });
    ...
    try {
      ...
      expect(db.prepare('SELECT version, name FROM execution_schema_migrations ORDER BY version').all()).toEqual([
        ...
        { version: 4, name: '004_execution_artifact_index' },
      ]);
    ...
    expect(reopenedStatus).toMatchObject({ priorUserVersion: 4, schemaVersion: 4, quickCheck: 'ok', integrity: 'ok' });
  });

  it('backs up and migrates an existing schema v1 database to v4 without losing existing runs or nodes', async () => {
    ...
    expect(opened).toMatchObject({ priorUserVersion: 1, schemaVersion: 4, integrity: 'ok' });
  ...
  it('backs up schema v2 before adding parsed-output and artifact persistence through v4', async () => {
    ...
    stores.push(store);
    const opened = await store.open({ dbPath: f.dbPath, busyTimeoutMs: 1_500 });
    expect(opened).toMatchObject({ priorUserVersion: 2, schemaVersion: 4, integrity: 'ok' });
    expect(opened.migrationBackupPath).toBeTruthy();
    await expect(access(opened.migrationBackupPath!)).resolves.toBeUndefined();
    ...
    const rows = await store.client.query<{ name: string }>("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('execution_parsed_outputs','execution_artifacts') ORDER BY name");
    expect(rows.map((row) => row.name)).toEqual(['execution_artifacts', 'execution_parsed_outputs']);
  });

  it('backs up schema v3 before adding the artifact index in v4', async () => {
    const f = await fixture('migrate-v3');
    await mkdir(path.dirname(f.dbPath), { recursive: true });
    const v3Sql = EXECUTION_SCHEMA_SQL
      .replace(/CREATE TABLE IF NOT EXISTS execution_artifacts \([\s\S]*?\n\);\n\n/, '')
      .replace(/CREATE INDEX IF NOT EXISTS idx_execution_artifacts_hash[\s\S]*?;/, '')
      .replace(/CREATE INDEX IF NOT EXISTS idx_execution_artifacts_type[\s\S]*?;/, '')
      .replace(/CREATE INDEX IF NOT EXISTS idx_execution_artifacts_run_node[\s\S]*?;/, '');
    const db = new DatabaseSync(f.dbPath);
    try {
      db.exec(v3Sql);
      for (const [version, name] of [
        [1, '001_initial_execution_fabric'],
        [2, '002_declared_artifact_evidence'],
        [3, '003_structured_output_evidence'],
      ] as const) {
        db.prepare('INSERT OR IGNORE INTO execution_schema_migrations(version, name, applied_at) VALUES (?, ?, ?)')
    ...
    stores.push(store);
    const opened = await store.open({ dbPath: f.dbPath, busyTimeoutMs: 1_500 });
    expect(opened).toMatchObject({ priorUserVersion: 3, schemaVersion: 4, integrity: 'ok' });
    expect(opened.migrationBackupPath).toBeTruthy();
    await expect(access(opened.migrationBackupPath!)).resolves.toBeUndefined();
    const backup = new DatabaseSync(opened.migrationBackupPath!, { readOnly: true });
    try {
      expect(Number(backup.prepare('PRAGMA user_version').get()!.user_version)).toBe(3);
      const tables = (backup.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all() as Array<{ name: string }>).map((row) => row.name);
      expect(tables).toContain('execution_parsed_outputs');
      expect(tables).not.toContain('execution_artifacts');
      expect(String(backup.prepare('PRAGMA quick_check').get()!.quick_check)).toBe('ok');
    } finally { backup.close(); }
  });

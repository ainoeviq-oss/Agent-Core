...
export interface MemoryWorkerOpenPayload {
  dbPath: string;
  busyTimeoutMs?: number;
}

export interface MemoryWorkerSqlOperation {
  kind: 'exec' | 'run' | 'query';
  sql: string;
  params?: SqlPrimitive[];
  mode?: 'all' | 'get';
...
export interface MemoryWorkerRequest {
  id: number;
  command: MemoryWorkerCommand;
  payload?: unknown;
}

export interface SerializedMemoryWorkerError {
  code: string;
  message: string;
}
...
export function createDatabaseWorkerSource(): string {
  return String.raw`
    ...
    function sendResult(id, result) {
      if (resultBytes(result) > maxResponseBytes) {
        parentPort.postMessage({
          id,
          ok: false,
          error: { code: 'MEMORY_RESULT_TOO_LARGE', message: 'Memory worker result exceeded the configured response bound' },
        });
        return;
      }
    ...
    function requireDb() {
      if (!db) {
        const error = new Error('Memory database is not open');
        error.code = 'MEMORY_DB_NOT_OPEN';
        throw error;
      }
      return db;
    }
    ...
    function executeOperation(database, operation) {
      if (!operation || typeof operation !== 'object' || typeof operation.kind !== 'string' || typeof operation.sql !== 'string') {
        const error = new Error('Invalid transaction operation');
        error.code = 'MEMORY_INVALID_OPERATION';
        throw error;
      }
      ...
      const error = new Error('Unsupported transaction operation kind');
      error.code = 'MEMORY_INVALID_OPERATION';
      throw error;
    }
    ...
    parentPort.on('message', (request) => {
      ...
      try {
        if (command === 'open') {
          if (db) {
            const error = new Error('Memory database is already open');
            error.code = 'MEMORY_DB_ALREADY_OPEN';
            throw error;
          }
          const payload = request.payload || {};
          if (typeof payload.dbPath !== 'string' || payload.dbPath.trim() === '') {
            const error = new Error('dbPath is required');
            error.code = 'MEMORY_DB_OPEN_FAILED';
            throw error;
          }
          try {
            ...
            const timeout = Number.isSafeInteger(payload.busyTimeoutMs) && payload.busyTimeoutMs > 0 ? payload.busyTimeoutMs : 5000;
        ...
        if (command === 'query') {
          ...
          sendResult(id, statement.all(...params));
          return;
        }
        ...
        if (command === 'transaction') {
          ...
          if (!Array.isArray(operations) || operations.length === 0 || operations.length > 1000) {
            ...
            error.code = 'MEMORY_INVALID_TRANSACTION';
            throw error;
          }
        ...
        const error = new Error('Unsupported memory worker command');
        error.code = 'MEMORY_UNKNOWN_COMMAND';
        throw error;
      } catch (error) {
        sendError(id, error, errorCode(error, 'MEMORY_WORKER_ERROR'));
      }
    });
  `;
}

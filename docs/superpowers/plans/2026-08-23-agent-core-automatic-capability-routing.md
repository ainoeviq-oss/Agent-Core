...
- [ ] **Step 3: Implement deterministic classification** using existing recommendation scores as the primary relevance signal plus bounded task-shape signals:
```ts
...
- [ ] **Step 4: Implement native-ready selection without relevance inversion.** A native-ready skill can become required only when score >= `Math.max(8, topScore * 0.65)`. Select at most 2 required skills.

- [ ] **Step 5: Implement verification policy.** Atomic read-like route may skip verification; atomic mutation gets cheap read-back; structured/domain/high-impact requires verification.

- [ ] **Step 6: Assert determinism** by deep-comparing repeated routing output for identical registry/task/context inputs.

- [ ] **Step 7: Commit** `feat: add deterministic Agent Core capability router`.
...
- [ ] **Step 1: Write failing lifecycle tests** for UUID route IDs, 30-minute expiry, principal ownership, loaded-skill state, required-skill enforcement, and 256-context pruning.
- [ ] **Step 2: Run `npm test -- tests/route-context-store.test.ts`** and verify RED.
- [ ] **Step 3: Implement storage with `crypto.randomUUID()` and `Map`**, never disk persistence.
- [ ] **Step 4: Implement stable `AgentCoreRouteError` codes:** `ROUTE_NOT_FOUND`, `ROUTE_EXPIRED`, `ROUTE_PRINCIPAL_MISMATCH`, `ROUTE_TOOL_NOT_ALLOWED`, `ROUTE_SKILL_REQUIRED`.
- [ ] **Step 5: Prune expired contexts opportunistically; when still >=256 before create, prune oldest until capacity exists.**
- [ ] **Step 6: Wire router/store into `createRuntimeServices()`** with the same registry instance.
- [ ] **Step 7: Run route-context + registry regression tests.**
- [ ] **Step 8: Commit** `feat: add principal-bound routing contexts`.
...
- [ ] **Step 1: Write failing tool-surface tests**: `capability_route` exists, public `capability_recommend` is absent, total tools remain 23.
- [ ] **Step 2: Write failing route-call test** and assert returned UUID/tier/mode/recommendations.
- [ ] **Step 3: Implement `capability_route` as a non-destructive but non-idempotent internal-state action.** MCP annotations must be `readOnlyHint: false`, `destructiveHint: false`, `idempotentHint: false`, `openWorldHint: false`, because each call issues a fresh route context.
- [ ] **Step 4: Extend `skill_load` input** to `{ id, routeContextId?: string }`. Existing audited loading happens first; only after success does `runtime.routes.markSkillLoaded(routeContextId, key.id, id)` run.
- [ ] **Step 5: Bump server to `0.5.0` and stage `v4-automatic-capability-routing`**, adding capability markers `routing.capability_route`, `routing.principal_bound_context`, `routing.execution_gate`.
- [ ] **Step 6: Run capability/MCP tests and TypeScript build.**
- [ ] **Step 7: Commit** `feat: expose Agent Core capability routing`.
...
- [ ] **Step 1: Write failing bypass tests** for all 11 tools using a fabricated UUID; expect `ROUTE_NOT_FOUND` and zero underlying side effect.
- [ ] **Step 2: Write two-principal test**: route created by A cannot drive `write_file` under B; expect `ROUTE_PRINCIPAL_MISMATCH`.
- [ ] **Step 3: Write required-skill test**: route requiring one native skill blocks execution until `skill_load(id, routeContextId)` succeeds.
- [ ] **Step 4: Implement `route-guard.ts`** so validation happens before filesystem/search/process service invocation.
- [ ] **Step 5: Add route field + dependency description** to the 11 MCP schemas: `Obtain routeContextId from capability_route before using this tool.`
- [ ] **Step 6: Preserve direct cleanup**: verify expired route context can never prevent `stop_process` on an existing process session.
- [ ] **Step 7: Update operational tests** to create and reuse a legitimate route for each coherent workflow.
- [ ] **Step 8: Run full operational/enforcement/build regression suite.**
- [ ] **Step 9: Commit** `feat: enforce capability routing before execution`.
...
- [ ] **Step 1: Extend plugin test** to require `capability_route`, `routeContextId`, and route-aware `skill_load` in the packaged Router Skill.
- [ ] **Step 2: Update Router Skill** to call `capability_route` before every route-required Agent Core operation without asking the user to name routing primitives.
- [ ] **Step 3: Reuse one route for one coherent user goal; create a new route only when the goal materially changes.**
- [ ] **Step 4: Required native-ready skills use `skill_load(id, routeContextId)`; reference-only metadata is never instruction-loaded.**
- [ ] **Step 5: Keep routing chatter internal** unless dependency/risk/failure information materially benefits the user.
- [ ] **Step 6: Update README/plugin docs** for v0.5 automatic routing and the one-time ChatGPT Scan Tools + Router Skill refresh after deployment.
- [ ] **Step 7: Run plugin tests and `npm run build:plugin`.**
- [ ] **Step 8: Commit** `docs: make Agent Core routing a native reflex`.
...
- [ ] **Step 1: Write failing audit tests** covering create/load/validate/reject flows.
- [ ] **Step 2: Use sentinel task text, secret, OAuth token, and skill body in tests** and assert none appear in routing audit output.
- [ ] **Step 3: Implement metadata-only routing audit helpers** alongside current request logging; do not duplicate transport logs.
- [ ] **Step 4: Run routing-audit + OAuth + secret-leak regression tests.**
- [ ] **Step 5: Commit** `feat: audit Agent Core routing decisions safely`.
...
- [ ] **Step 1: Extend smoke test for atomic flow:** `capability_route("Create a small proof file")` -> route-bound `write_file` -> route-bound `read_file`; assert `atomic_direct` and no required skill.
- [ ] **Step 2: Add real-registry domain flow:** route `Improve a frontend dashboard visual hierarchy and spacing`; assert frontend-relevant recommendation and, when route requires a native-ready skill, load it with the same route ID before a harmless route-bound operation.
- [ ] **Step 3: Add bypass smoke:** random route UUID is rejected with no side effect.
- [ ] **Step 4: Run pre-cutover gates in isolated worktree:** full tests, build, plugin build, brand scan, non-production smoke.
- [ ] **Step 5: Review tool schema diff:** exactly one capability tool rename and exactly 11 operational schemas gain route IDs; no unrelated permission/tool expansion.
- [ ] **Step 6: Present implementation results to user and obtain explicit approval before merging/restarting production.**
- [ ] **Step 7: After approval, merge/push private `main`, restart Agent Core v0.5.0, preserve rollback to verified v0.4.0.**
- [ ] **Step 8: Refresh/Scan Tools once in ChatGPT Agent Core app** and update/install generated Router Skill.
- [ ] **Step 9: Fresh-chat complex test with no routing jargon:** `@Agent Core perbaiki struktur dan kualitas frontend project ini, cek dulu projectnya lalu kerjakan dan verifikasi hasilnya.` Success requires internal `capability_route` -> optional `skill_load` -> route-bound execution -> verification.
- [ ] **Step 10: Fresh-chat atomic test with no routing jargon:** `@Agent Core buat file route-proof.txt berisi "Agent Core automatic routing works".` Success requires internal route -> `atomic_direct` -> route-bound write, with no unrelated skill load.

...
- [x] **Step 1: Write failing parser tests** for Skill/Agent/Command/Hook rows, duplicate names, source URL extraction, compatibility/language parsing, and deterministic IDs.
- [x] **Step 2: Run parser tests** and verify RED from missing parser/types.
- [x] **Step 3: Implement minimal parser/types** that scan every `categories/*.md` table row and map section headings to canonical capability types.
- [x] **Step 4: Add failing atomic-registry test** proving a rejected generation leaves the previous `catalog.json` untouched.
- [x] **Step 5: Implement staged registry publication** with `catalog.json`, per-item JSON, `coverage.json`, and catalog SHA metadata.
- [x] **Step 6: Run parser tests + build** and expect green.
- [x] **Step 7: Run the real catalog sync/parser** and assert `coverage.total === catalog.items.length`, with every item having name/type/category/declaredPurpose/source/state.
- [x] **Step 8: Commit** with `feat: index Korean agent capability catalog`.
...
- [x] **Step 1: Write failing service tests** for query/category/type/risk filtering, deterministic ranking, metadata-only recommendation, coverage, and native-ready load enforcement.
- [x] **Step 2: Run `npm test -- tests/capability-registry.test.ts`** and verify RED.
- [x] **Step 3: Implement the registry service** with tokenized lexical scoring over name, aliases, purpose, functional summary, category, triggers, and compatibility; never include full skill instructions in search/recommend.
- [x] **Step 4: Re-run service tests and full build**; expect green.
- [x] **Step 5: Wire the registry into `RuntimeServices`** using `config.capabilityDir` with graceful empty-registry behavior when no generated catalog exists.
- [x] **Step 6: Commit** with `feat: add deferred capability registry service`.
...
- [x] **Step 1: Write failing audit/normalizer tests** using local fixture source + LICENSE for permissive, unknown-license, and dangerous-hook cases.
- [x] **Step 2: Run audit tests** and verify RED.
- [x] **Step 3: Implement source audit and normalization gates**; source text is preserved, summaries are separate metadata, and high/unknown risk cannot become native-ready.
- [x] **Step 4: Resolve one real catalog Skill from a directly linked `SKILL.md` whose repository license is explicitly permissive; prefer an `anthropics/skills` catalog entry when present. If none is present in the parsed catalog, select the first direct Skill source with an SPDX-recognizable permissive license. Do not substitute unknown-license material.**
- [x] **Step 5: Cache exact source + license + commit SHA, analyze actual instructions, generate normalized native skill metadata, and update only that item's registry state to `native_ready`.**
- [x] **Step 6: Re-run audit tests, parser coverage validation, and build**; expect green.
- [x] **Step 7: Commit source-code changes only** with `feat: audit and normalize native skill candidates`; runtime caches/normalized generated artifacts remain ignored.
...
- [x] **Step 1: Write failing MCP discovery tests** asserting the 17 operational tools remain present plus six capability tools with read-only annotations.
- [x] **Step 2: Write failing call tests** for recommendation, search, get, coverage and rejected non-native skill load.
- [x] **Step 3: Run MCP capability tests** and verify RED.
- [x] **Step 4: Register six capability tools** with bounded schemas/results and no unrelated instruction expansion.
- [x] **Step 5: Update server version/capability report** to the hybrid-registry release while preserving existing auth/workspace behavior.
